...
void setup() {
  ...
  beat = minim.loadFile("beat.aif", 2048);
  beat.loop();
}
...
void draw() {
  ...
  if (humans.size() > 0) {
    ...
    float noseY = keypoints.getJSONArray(ModelUtils.POSE_NOSE_INDEX).getFloat(1);
    if (noseY < 0.5) {
      beat.mute();
    } else {
      beat.unmute();
...
// this is called each time Procesdrsing connects to Runway
